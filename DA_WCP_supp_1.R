...
# theta_mat, eta_mat : 5*4

theta_mat <- cbind(c(1, 1.2, 1.4, 1.6, 1.8), c(1, 1.2, 1.4, 1.6, 1.8), c(-1, -1.2, -1.4, -1.6, -1.8), c(-1, -1.2, -1.4, -1.6, -1.8))
eta_mat <- cbind(c(1, 1.2, 1.4, 1.6, 1.8), c(1.8, 1.6, 1.4, 1.2, 1), c(3, 3.2, 3.4, 3.6, 3.8), c(3.8, 3.6, 3.4, 3.2, 3))

# --------- Lists to save simulation results ---------

df_m = vector("list", 4) # list for marginal methods
df_g = vector("list", 4) # list for group-conditional methods
rvec_m = vector("list", 4)
rvec_g = vector("list", 4)
...
generate_data_supp_1 <- function(m, t, num, sim, test = F){
  if(test){
    n <- m
  ...
  for (k in 1:5) {
    idx <- which(Z == k)
    if(num != 4){
      if(t == 1) X[idx] <- rbeta(length(idx), alpha1[k], beta1[k])
      if(t == 2) X[idx] <- rbeta(length(idx), alpha2[k], beta2[k])
    }
    if(num == 4) X[idx] <- rbeta(length(idx), alpha1[k], beta1[k])
    
    for(i in idx){
      if(num != 5) Y[i] <- rnorm(1, 0, sd = X[i]*theta_mat[k, sim] + eta_mat[k, sim])
...
DA_WCP_supp_1 <- function(m1, m2, num, sim, M = 100, K = 5, R = 4, alpha = 0.2, n_alpha = 5, rvec = F, 
  ...
  for(m in 1:M){
    ...
    # Data generation
    set.seed(123+m)
    data1 <- generate_data_supp_1(m1, t = 1, num, sim)
    data2 <- generate_data_supp_1(m2, t = 2, num, sim)
    data_test <- generate_data_supp_1(5000, t = 2, num, sim, test = T)
...
for(sim in 1:4){
  for(num in 1:5){
    sim_num <- DA_WCP_supp_1(m1, m2, num, sim, rvec = F)
    ...
    save.image(file = "DA_WCP_supp_1.RData")
  }
}
...
# Add new column
sim = 4 # change this from 1 ~ 4 to generate figures supp1a~supp1d
...
pdf(file = "simulation_coverage_m_supp_1_d.pdf", width = 13, height = 3.5)
...
pdf(file = "simulation_length_m_supp_1_d.pdf", width = 13, height = 3.5)
...
pdf(file = "simulation_coverage_g_supp_1_d.pdf", width = 12, height = 7)
